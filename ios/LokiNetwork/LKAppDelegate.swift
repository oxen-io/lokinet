...
final class LKAppDelegate : UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey:Any]? = nil) -> Bool {
        ...
        tunnelProviderManager.loadFromPreferences { [tunnelProviderManager] error in
            guard error == nil else { return print("[Loki] Couldn't load the tunnel configuration due to error: \(error).") }
            ...
            tunnelProviderManager.saveToPreferences { error in
                guard error == nil else { return print("[Loki] Couldn't save the tunnel configuration due to error: \(error).") }
                tunnelProviderManager.loadFromPreferences { error in
                    guard error == nil else { return print("[Loki] Couldn't load the tunnel configuration due to error: \(error).") }
                    do {
                        try tunnelProviderManager.connection.startVPNTunnel()
                    } catch (let error) {
                        print("[Loki] Couldn't open the tunnel due to error: \(error).")
                    }
                }
            }
        }
