...
final class LKTunnel : NSObject {
    ...
    func open() -> Promise<Void> {
        ...
        LKLog("Tunnel (to \(configuration.address):\(configuration.port) state changed to preparing.")
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey:Any]?, context: UnsafeMutableRawPointer?) {
        if keyPath == "state" && context?.assumingMemoryBound(to: Optional<NWTCPConnection>.self).pointee == session {
            ...
            LKLog("Tunnel (to \(configuration.address):\(configuration.port) state changed to \(state).")
            switch state {
            case .ready:
                openSeal!.fulfill(())
                openSeal = nil
    ...
    private func read(_ datagrams: [Data]?, _ error: Error?) {
        if let datagrams = datagrams {
            LKLog("Read datagrams: \(datagrams).")
        } else if let error = error {
            LKLog("Couldn't read datagrams due to error: \(error).")
        } else {
            LKLog("Couldn't read datagrams.")
        }
    }
