...
namespace std
{
    ...
    {
        size_t operator()(const std::pair<llarp::RouterID, llarp::PathID_t>& i) const
        {
...
namespace llarp
{
    ...
    namespace path
    {
        ...
        struct PathSet
        {
            ...
            virtual std::shared_ptr<PathSet> GetSelf() = 0;
            ...
            virtual void Build(std::vector<RemoteRC> hops, PathRole roles = ePathRoleAny) = 0;
            ...
            virtual void HandlePathBuilt(std::shared_ptr<Path> path) = 0;

            virtual void HandlePathBuildTimeout(std::shared_ptr<Path> path);

            virtual void HandlePathBuildFailedAt(std::shared_ptr<Path> path, RouterID hop);

            void PathBuildStarted(std::shared_ptr<Path> path);

            /// a path died now what?
            virtual void HandlePathDied(std::shared_ptr<Path> path);

            bool GetNewestIntro(service::Introduction& intro) const;

            void AddPath(std::shared_ptr<Path> path);

            std::shared_ptr<Path> GetByUpstream(RouterID remote, PathID_t rxid) const;

            void ExpirePaths(llarp_time_t now, Router* router);
            ...
            virtual bool ShouldPublishDescriptors([[maybe_unused]] llarp_time_t now) const
            {
                return false;
            }
            ...
            std::shared_ptr<Path> GetByEndpointWithID(RouterID router, PathID_t id) const;
            ...
            virtual void send_packet_to_remote(std::string buf) = 0;

            virtual std::optional<std::vector<RemoteRC>> GetHopsForBuild() = 0;

            void ForEachPath(std::function<void(const std::shared_ptr<Path>&)> visit) const
            {
            ...
            void UpstreamFlush(Router* r);

            void DownstreamFlush(Router* r);
