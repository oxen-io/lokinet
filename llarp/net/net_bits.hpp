...
namespace llarp
{
    ...
    /// get a netmask with the higest numset bits set
    constexpr uint32_t _netmask_ipv4_bits(uint32_t numset)
    {
        return (32 - numset) ? (1 << numset) | _netmask_ipv4_bits(numset + 1) : 0;
    }

    /// get a netmask given some /N range
    constexpr huint32_t netmask_ipv4_bits(uint32_t num)
    {
    ...
    namespace net
    {
        inline auto ipaddr_netmask_bits(uint32_t bits, int af)
        {
            if (af == AF_INET6)
