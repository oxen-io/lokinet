...
namespace llarp::service
{
    ...
    // public std::enable_shared_from_this<Endpoint>
    {
        ...
        void QueueRecvData(RecvDataEvent ev);
        ...
        virtual util::StatusObject ExtractStatus() const;
        ...
        std::unordered_set<AddressVariant_t> AllRemoteEndpoints() const override;

        bool ShouldPublishDescriptors(llarp_time_t now) const override;
        ...
        bool HandleDataDrop(std::shared_ptr<path::Path> p, const PathID_t& dst, uint64_t s);
        ...
        void PutSenderFor(const ConvoTag& tag, const ServiceInfo& info, bool inbound);

        bool HasInboundConvo(const Address& addr) const;

        bool HasOutboundConvo(const Address& addr) const;

        bool GetCachedSessionKeyFor(const ConvoTag& remote, SharedSecret& secret) const;

        void PutCachedSessionKeyFor(const ConvoTag& remote, const SharedSecret& secret);

        bool GetSenderFor(const ConvoTag& remote, ServiceInfo& si) const;

        void PutIntroFor(const ConvoTag& remote, const Introduction& intro);

        bool GetIntroFor(const ConvoTag& remote, Introduction& intro) const;

        void RemoveConvoTag(const ConvoTag& remote);

        void ConvoTagTX(const ConvoTag& remote);

        void ConvoTagRX(const ConvoTag& remote);

        void PutReplyIntroFor(const ConvoTag& remote, const Introduction& intro);

        bool GetReplyIntroFor(const ConvoTag& remote, Introduction& intro) const;

        bool GetConvoTagsForService(const Address& si, std::set<ConvoTag>& tag) const;
        ...
        void ResetConvoTag(ConvoTag tag, std::shared_ptr<path::Path> path, PathID_t from);

        const std::set<RouterID>& SnodeBlacklist() const;

        // Looks up the ConvoTag and, if it exists, calls SendToOrQueue to send it to a remote
        // client or a snode (or nothing, if the convo tag is unknown).
        bool send_to(ConvoTag tag, std::string payload) override;
