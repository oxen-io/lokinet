...
namespace llarp::service
{
    ...
    bool Endpoint::HasPendingPathToService(const Address& addr) const
    {
    ...
    std::optional<std::variant<Address, RouterID>> Endpoint::GetEndpointWithConvoTag(ConvoTag tag) const
    {
             ...
             poker = router()->route_poker()](std::string name_result, bool success) mutable {
                ...
                if (auto saddr = service::Address(); saddr.FromString(name_result))
                {
                    ...
                    auto result = ptr->EnsurePathToService(
                        saddr,
                        [ptr, name, name_result, ranges, result_handler, poker](auto addr, OutboundContext* ctx) {
                            if (ctx == nullptr)
                            {
                                result_handler(false, "could not establish flow to {} ({})"_format(name_result, name));
                                return;
                            }
    ...
    void Endpoint::LookupServiceAsync(
        std::string name, std::string service, std::function<void(std::vector<dns::SRVData>)> resultHandler)
    {
        ...
        // then we invoke the resultHandler with an empty vector.
        lookup_name(
            name, [this, resultHandler, service = std::move(service)](std::string name_result, bool success) mutable {
                ...
                const auto build_started = EnsurePathTo(
                    saddr,
                    [this, address = std::move(saddr), resultHandler, service = std::move(service)](auto maybe_tag) {
                        if (not maybe_tag)
                            return resultHandler({});

                        // we can probably get this info before we have a path to them but we do
                        // this after we have a path so when we send the DNS response back they can
    ...
    void Endpoint::Tick(llarp_time_t)
    {
        ...
        EndpointUtil::TickRemoteSessions(now, _state->remote_sessions, _state->dead_sessions, Sessions());
        // expire convotags
        ...
        if (NumInStatus(path::PathStatus::ESTABLISHED) > 1)
        {
            for (const auto& item : _startup_ons_mappings)
            {
                ...
                lookup_name(name, [this, name, info = item.second](std::string name_result, bool success) mutable {
                    if (not success)
                        return;
    ...
    void Endpoint::PutSenderFor(const ConvoTag& tag, const ServiceInfo& info, bool inbound)
    {
        if (info.Addr().IsZero())
        {
        ...
        if (itr == Sessions().end())
        {
            ...
            {
                LogWarn(
                    Name(), " not adding sender for ", info.Addr(), " session is inbound and we want outbound T=", tag);
                return;
            }
    ...
    bool Endpoint::GetSenderFor(const ConvoTag& tag, ServiceInfo& si) const
    {
        auto itr = Sessions().find(tag);
        if (itr == Sessions().end())
            return false;
    ...
    void Endpoint::PutIntroFor(const ConvoTag& tag, const Introduction& intro)
    {
        auto& s = Sessions()[tag];
        s.intro = intro;
    }

    bool Endpoint::GetIntroFor(const ConvoTag& tag, Introduction& intro) const
    {
        auto itr = Sessions().find(tag);
        if (itr == Sessions().end())
            return false;
    ...
    void Endpoint::PutReplyIntroFor(const ConvoTag& tag, const Introduction& intro)
    {
        auto itr = Sessions().find(tag);
        if (itr == Sessions().end())
        {
    ...
    bool Endpoint::GetReplyIntroFor(const ConvoTag& tag, Introduction& intro) const
    {
        auto itr = Sessions().find(tag);
        if (itr == Sessions().end())
            return false;
    ...
    bool Endpoint::GetCachedSessionKeyFor(const ConvoTag& tag, SharedSecret& secret) const
    {
        auto itr = Sessions().find(tag);
        if (itr == Sessions().end())
            return false;
    ...
    void Endpoint::PutCachedSessionKeyFor(const ConvoTag& tag, const SharedSecret& k)
    {
        auto itr = Sessions().find(tag);
        if (itr == Sessions().end())
        {
    ...
    void Endpoint::ConvoTagTX(const ConvoTag& tag)
    {
        if (Sessions().count(tag))
            Sessions()[tag].TX();
    }

    void Endpoint::ConvoTagRX(const ConvoTag& tag)
    {
        if (Sessions().count(tag))
            Sessions()[tag].RX();
    }

    bool Endpoint::LoadKeyFile()
    {
    ...
    void Endpoint::regen_and_publish_introset()
    {
        ...
        if (const auto maybe = GetCurrentIntroductionsWithFilter([now](const service::Introduction& intro) -> bool {
    ...
    size_t Endpoint::UniqueEndpoints() const
    {
    ...
    void Endpoint::ResetInternalState()
    {
        path::PathBuilder::ResetInternalState();
        static auto resetState = [](auto& container, auto getter) {
            std::for_each(
                container.begin(), container.end(), [getter](auto& item) { getter(item)->ResetInternalState(); });
        };
    ...
    bool Endpoint::ShouldPublishDescriptors(llarp_time_t now) const
    {
        ...
        const auto next_pub = lastEventAt
            + (_state->local_introset.HasStaleIntros(now, path::INTRO_STALE_THRESHOLD) ? IntrosetPublishRetryCooldown
    ...
    std::optional<std::vector<RemoteRC>> Endpoint::GetHopsForBuildWithEndpoint(RouterID endpoint)
    {
    ...
    void Endpoint::PutNewOutboundContext(const service::IntroSet& introset, llarp_time_t)
    {
    ...
    void Endpoint::lookup_name(std::string name, std::function<void(std::string, bool)> func)
    {
    ...
    void Endpoint::HandlePathBuilt(std::shared_ptr<path::Path> p)
    {
    ...
    bool Endpoint::HandleDataDrop(std::shared_ptr<path::Path> p, const PathID_t& dst, uint64_t seq)
    {
        LogWarn(Name(), " message ", seq, " dropped by endpoint ", p->Endpoint(), " via ", dst);
        return true;
    }

    std::unordered_map<std::string, std::string> Endpoint::NotifyParams() const
    {
        return {{"LOKINET_ADDR", _identity.pub.Addr().ToString()}};
    }

    void Endpoint::FlushRecvData()
    {
    ...
    void Endpoint::QueueRecvData(RecvDataEvent ev)
    {
    ...
    EndpointBase::AddressVariant_t Endpoint::LocalAddress() const
    {
    ...
    std::unordered_set<EndpointBase::AddressVariant_t> Endpoint::AllRemoteEndpoints() const
    {
    ...
    void Endpoint::RemoveConvoTag(const ConvoTag& t)
    {
        Sessions().erase(t);
    }

    void Endpoint::ResetConvoTag(ConvoTag tag, std::shared_ptr<path::Path> p, PathID_t /* from */)
    {
    ...
    void Endpoint::HandlePathDied(std::shared_ptr<path::Path> p)
    {
    ...
    bool Endpoint::WantsOutboundSession(const Address& addr) const
    {
    ...
    bool Endpoint::EnsurePathTo(
        std::variant<Address, RouterID> addr, std::function<void(std::optional<ConvoTag>)> hook, llarp_time_t timeout)
    {
    ...
    bool Endpoint::EnsurePathToSNode(
        const RouterID snode, std::function<void(const RouterID, std::shared_ptr<exit::BaseSession>, ConvoTag)> hook)
    {
    ...
    void Endpoint::SRVRecordsChanged()
    {
    ...
    bool Endpoint::send_to(ConvoTag tag, std::string payload)
    {
        if (tag.IsZero())
        {
    ...
    void Endpoint::Pump(llarp_time_t)
    {
        ...
        while (not queue.empty())
        {
            ...
            log::debug(logcat, "{} handling inbound packet (size {}B) on tag {}", Name(), msg.tag, msg.payload.size());
    ...
    std::optional<ConvoTag> Endpoint::GetBestConvoTagFor(std::variant<Address, RouterID> remote) const
    {
            ...
            for (const auto& [tag, session] : Sessions())
            {
                ...
                if (session.remote.Addr() == *ptr)
                {
                    ...
                    if (session.inbound)
                    {
                        ...
                        // if we have no path to the remote router that's fine still use it just in
                        // case this is the ONLY one we have
                        if (path == nullptr)
                        {
                            ret = tag;
                            continue;
    ...
    bool Endpoint::HasConvoTag(const ConvoTag& t) const
    {
        return Sessions().find(t) != Sessions().end();
    }

    std::optional<uint64_t> Endpoint::GetSeqNoForConvo(const ConvoTag& tag)
    {
        auto itr = Sessions().find(tag);
        if (itr == Sessions().end())
            return std::nullopt;
    ...
    bool Endpoint::ShouldBuildMore(llarp_time_t now) const
    {
        ...
        if (NumInStatus(path::PathStatus::BUILDING) >= requiredPaths)
            return false;
        return NumPathsExistingAt(now + (path::DEFAULT_LIFETIME - path::INTRO_PATH_SPREAD)) < requiredPaths;
    }
    ...
    const EventLoop_ptr& Endpoint::Loop()
    {
        return router()->loop();
    }

    void Endpoint::BlacklistSNode(const RouterID snode)
    {
    ...
    const IntroSet& Endpoint::intro_set() const
    {
        return _state->local_introset;
    }

    IntroSet& Endpoint::intro_set()
    {
        return _state->local_introset;
    }

    const std::unordered_map<ConvoTag, Session>& Endpoint::Sessions() const
    {
        return _state->m_Sessions;
    }

    std::unordered_map<ConvoTag, Session>& Endpoint::Sessions()
    {
        return _state->m_Sessions;
    }

    void Endpoint::SetAuthInfoForEndpoint(Address addr, AuthInfo info)
    {
        if (info.token.empty())
        {
    ...
    std::optional<AuthInfo> Endpoint::MaybeGetAuthInfoForEndpoint(Address remote)
    {
