  ...
  namespace rpc
  {
    ...
    JsonRpcDispatcher::processRaw(const std::string& input) const
    {
      ...
      try {
        ...
        auto error = createJsonRpcErrorObject(ERROR_PARSE, "Invalid JSON string", {{"parseMessage", e.what()}});
    ...
    JsonRpcDispatcher::process(const nlohmann::json& obj) const
    {
      ...
      {
        auto error = createJsonRpcErrorObject(ERROR_INVALID_REQUEST, "Request not a proper JSON object");
        return createJsonRpcResponseObject(nullptr, nullptr, error);
      }
      ...
      if (response.isError)
      {
        auto error = createJsonRpcErrorObject(ERROR_INTERNAL, "Internal error", response.payload);
        return createJsonRpcResponseObject(id, nullptr, error);
      }
      else
      {
    ...
    void
    JsonRpcDispatcher::setHandler(const std::string& method, DispatchHandler handler)
    {
    ...
    nlohmann::json
    JsonRpcDispatcher::createJsonRpcErrorObject(int code, std::string message, nlohmann::json data)
    {
      nlohmann::json obj;
    ...
    nlohmann::json
    JsonRpcDispatcher::createJsonRpcResponseObject(nlohmann::json id, nlohmann::json result, nlohmann::json error)
    {
      nlohmann::json obj;
      ...
      if (error != nullptr && result != nullptr) {
        LogWarn("Warning: caller provided both error and result to createJsonRpcResponseObject(), ignoring result");
      }
