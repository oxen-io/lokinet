...
namespace llarp
{
    ...
    bool RouterProfile::IsGoodForConnect(uint64_t chances) const
    {
        return checkIsGood(connectTimeoutCount, connectGoodCount, chances);
    }

    bool RouterProfile::IsGoodForPath(uint64_t chances) const
    {
    ...
    bool Profiling::IsBadForConnect(const RouterID& r, uint64_t chances)
    {
    ...
    bool Profiling::IsBadForPath(const RouterID& r, uint64_t chances)
    {
        if (m_DisableProfiling.load())
            return false;
        util::Lock lock{m_ProfilesMutex};
    ...
    bool Profiling::IsBad(const RouterID& r, uint64_t chances)
    {
        if (m_DisableProfiling.load())
            return false;
        util::Lock lock{m_ProfilesMutex};
    ...
    void Profiling::MarkConnectTimeout(const RouterID& r)
    {
        util::Lock lock{m_ProfilesMutex};
    ...
    void Profiling::MarkConnectSuccess(const RouterID& r)
    {
        util::Lock lock{m_ProfilesMutex};
        auto& profile = m_Profiles[r];
    ...
    void Profiling::ClearProfile(const RouterID& r)
    {
        util::Lock lock{m_ProfilesMutex};
        m_Profiles.erase(r);
    }

    void Profiling::MarkHopFail(const RouterID& r)
    {
        util::Lock lock{m_ProfilesMutex};
        auto& profile = m_Profiles[r];
    ...
    void Profiling::MarkPathFail(path::Path* p)
    {
        util::Lock lock{m_ProfilesMutex};
        bool first = true;
    ...
    void Profiling::MarkPathTimeout(path::Path* p)
    {
        util::Lock lock{m_ProfilesMutex};
        for (const auto& hop : p->hops)
        {
    ...
    void Profiling::MarkPathSuccess(path::Path* p)
    {
        util::Lock lock{m_ProfilesMutex};
    ...
    bool Profiling::ShouldSave(llarp_time_t now) const
    {
