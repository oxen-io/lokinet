...
namespace llarp::handlers
{
    ...
    util::StatusObject TunEndpoint::ExtractStatus() const
    {
        ...
        dns::Message msg, std::function<void(dns::Message)> reply)
    {
        ...
        auto ReplyToLokiDNSWhenReady = [this, reply, timeout = PathAlignmentTimeout()](
            ...
            if (HasInboundConvo(addr))
            {
                ...
                // have a state race this codepath is hit when an application verifies that reverse
                // and forward dns records match for an inbound session
    ...
    std::unordered_map<std::string, std::string> TunEndpoint::NotifyParams() const
    {
    ...
    void TunEndpoint::Tick(llarp_time_t now)
    {
        Endpoint::Tick(now);
    }
    ...
    void TunEndpoint::HandleGotUserPacket(net::IPPacket pkt)
    {
        huint128_t dst, src;
        if (pkt.IsV4())
        {
