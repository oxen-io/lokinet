...
crypto_threadworker_verifyrc(void *user)
{
  ...
  verify_request->valid =
      llarp_rc_verify_sig(verify_request->crypto, &verify_request->rc);
                          ...
                          struct llarp_async_verify_rc *job)
{
  ...
  // we expect the following to be already set up at this point: user (context:
  // router, llarp_link_establish_job), rc, hook do additional job set up
  /*
  job->logic = logic;
  job->crypto = crypto;
  ...
  */
  ...
  llarp_threadpool_queue_job(cryptoworker,
