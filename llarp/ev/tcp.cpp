...
namespace llarp
{
    ...
    static void tcp_write_cb(struct bufferevent* bev, void *user_arg)
    {
        auto *conn = reinterpret_cast<TCPConnection *>(user_arg);
    ...
    static void tcp_event_cb(struct bufferevent *bev, short what, void *user_arg)
    {
        ...
        log::log(logcat, what & BEV_EVENT_ERROR ? log::Level::err : log::Level::debug,
                ...
                what & BEV_EVENT_CONNECTED ? "CONNECTED" : "IMPOSSIBLE");
    ...
    TCPConnection::TCPConnection(struct bufferevent *_bev, evutil_socket_t _fd, std::shared_ptr<oxen::quic::Stream> s)
        : bev{_bev}, fd{_fd}, stream{s}
    {
        stream->set_stream_data_cb([this, _bev](oxen::quic::Stream& stream, std::span<const std::byte> data) mutable {
            ...
            else { // we got unpaused, but clogged the (kernel?) buffer again, pause again
                stream.pause();
                pending_buffer.insert(pending_buffer.end(), data.begin(), data.end());
                return;
            }
            ...
            log::debug(
                logcat,
                "Stream (id:{}) wrote {}B to TCP buffer",
                stream.stream_id(),
                data.size());
        });
    }
    ...
    std::shared_ptr<TCPConnection> TCPHandle::connect(event_base* _ev, oxen::quic::Address src, std::shared_ptr<oxen::quic::Stream> s, uint16_t port)
    {
        ...
        // by a single thread.
        struct bufferevent *_bev = bufferevent_socket_new(_ev, -1, BEV_OPT_CLOSE_ON_FREE);
